...
export function MindsetRingCard({
  ...
  return (
    <div className={className ? `${styles.mcard} ${className}` : styles.mcard}>
        ...
        >
          ...
          {arcs.map((arc) => (
            <g key={`arc-${arc.key}`} style={{ filter: `drop-shadow(0 0 5px ${arc.self}80)` }}>
              {arc.hasMid && (
                <path className={styles.arc} d={arcPath(R, arc.midStart, arc.midStop)} stroke={arc.self} />
              )}
              <path className={styles.arc} d={arc.tailA.d} stroke={`url(#mrc-ta-${arc.key})`} />
              <path className={styles.arc} d={arc.tailB.d} stroke={`url(#mrc-tb-${arc.key})`} />
            </g>
          ))}
          {arcs.map((arc) => (
            <circle
